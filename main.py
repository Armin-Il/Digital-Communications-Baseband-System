...
def load_audio(path, target_fs=16000):
    ...
    if x.ndim > 1:
        x = x.mean(axis=1)  # bordim roye mono
...
def generate_synthetic_test_audio(fs=16000, seconds=3.0, seed=0):
    #age voice ra peyda nakard mire khodesh sample misaze 
...
def run_full_project(audio_path=None, out_dir="results", B_list=(8, 4, 2),
    ...
    if audio_path and os.path.exists(audio_path):
        ...
        log(f"file part 1 {audio_path} | fs={fs}Hz | lenght={len(x)/fs:.2f}s")
    else:
        fs, x = generate_synthetic_test_audio()
        log("Hoshdar file voice ra shenasayi nakardim file masnoui gozashtim alan file ra dorost konid badan ")
    save_wav(os.path.join(out_dir, "00_input_normalized.wav"), fs, x)
 
    log("\n part 2 Quantization yeknavakht")
    uniform_results = {}
    for B in B_list:
        xq, idx, step = uniform_quantize(x, B)
    ...
    # MU-LAW
    log("\n part 2 Quantization Mu-Law (companding)")
    ok_inv, max_err = verify_companding_invertible()
    log(f"  test of maakos paziri F^-1(F(x))≈x : {'OK' if ok_inv else 'FAIL'} (max_err={max_err:.2e})")
 
    mulaw_results = {}
    log(f"\n  {'B':>3} | {'SQNR Uniform(dB)':>18} | {'SQNR mu-Law(dB)':>16} | {'difference':>8}")
    for B in B_list:
        ...
        log(f"  {B:>3} | {uniform_results[B]['SQNR_dB']:>18.2f} | {sqnr_mu:>16.2f} | {diff:>8.2f}")
 
    # jaryane Bit
    log("\n Part 4 entekhab behtarin jaryan : B=8)")
    best_B = 8
    ...
    log(f"  entekhabi : {'Mu-Law' if best_use_mu else 'Uniform'} | B={best_B}")
    log(f"  test maakos paziri : {'OK' if ok_bits else 'FAIL'}")
    log(f"  kol Bit ha : {len(bits)} | Rb = B*Fs = {best_B*fs} bit/s")
 
    # bakhsh Line coding
    log("\n part 5 Line Coding (Polar NRZ / Polar RZ / Manchester)")
    sps = 8
    Tb_sim = 1.0
    fs_sim = sps / Tb_sim
    line_bw = {}
    ...
    for code in ["nrz", "rz", "manchester"]:
        ...
        line_bw[code] = bw
        log(f"  {code.upper():<11}: band width (-3dB) ≈ {bw:.3f} x Rb | sefr teifi dar DC: {'yes' if dc_null else 'no'}")
    if line_bw.get("manchester", 0) > 0 and line_bw.get("rz", 1) > 0:
        log(f"   BW(Manchester)/BW(RZ) ≈ {line_bw['manchester']/line_bw['rz']:.2f}  (expect ≈ 2)")
 
    # AWGN +BER
    log("\n part 6 , 7 channel AWGN , BER  moghabele Eb/N0")
    ...
    log(f"\n  dar pooshe ha nemoudar ha va voice ha hastand : {out_dir}")
    return out_dir
